...
# Initialize an empty list to store the runtime
runtimes = []

# List of different transactions
transactions = [500,1000,1500,2000,2500,3000,3500,4000,4500,5000,5500,6000,6500,7000,7400]
import time
                    ...
                    if (n != None):
                        ...
                        if ((ant[len(j)][j] / ant[l][n]) >= minconf):
                            ...
                            if (closed.__contains__((n))):
                                c = c + 1
                                if (len(r) == 1):
                                    print(n, "(", ant[l][n], ")", "--->", r2, "(", ant[len(r)][r2], ")", " - conf(",
                                          (ant[len(j)][j] / ant[l][n]), ")")
                                else:
                                    print(n, "(", ant[l][n], ")", "--->", r, "(", ant[len(r)][r], ")", " conf(",
                                          (ant[len(j)][j] / ant[l][n]), ")")
    end_time = time.time()
...
print(runtimes)
print(transactions)
run = [0.6290509700775146, 0.6630551815032959, 0.6430511474609375, 0.745063066482544, 0.7160563468933105, 0.6650564670562744, 0.6890559196472168, 0.6950583457946777, 0.6760540008544922, 0.6840569972991943, 0.5930478572845459, 0.661055326461792, 0.6870570182800293, 0.6970574855804443, 0.6890552043914795, 0.6990580558776855, 0.6860575675964355, 0.69205641746521]
tran= [500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000, 6500, 7000, 7500, 8000, 8500, 9000]
# runt= [0.007000446319580078,0.0010001659393310547, 0.0010001659393310547, 0.0]
# import matplotlib.pyplot as plt
# # Plot the runtime
# plt.plot(transactions, runtimes)
# plt.xlabel('Transactions')
# plt.ylabel('Runtime (seconds)')
# #plt.title(f"Apriory input S={minsup} C={minconf}")
# plt.title(f"Apriory input S={minsup} C={minconf}")
#
# plt.show()

FPgrowth_runtime =runtimes
FPgrowth_transactions = transactions